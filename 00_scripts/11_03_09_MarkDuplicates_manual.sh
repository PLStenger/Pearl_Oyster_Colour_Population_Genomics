...
file=HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted.bam


# 1) Marking duplicates and removing them
cd ${DATA}
# module load java
time java -jar -Djava.io.tmpdir=$TMP ${PICARD_TOOLS}/MarkDuplicates.jar I=${file} O=${OUTDIR}/HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD.bam M=${OUTDIR}/HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_metrics.txt ASSUME_SORTED=TRUE VALIDATION_STRINGENCY=SILENT REMOVE_DUPLICATES=TRUE CREATE_INDEX=TRUE ;

file=${OUTDIR}/HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD.bam

# 2) Supplementary step to prevent bug: sorting & indexing bam files
cd ${OUTDIR}
$SAMTOOLS
samtools sort HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD.bam > HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted.bam ;
samtools index HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted.bam > HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted.bai  ;
...
cd ${OUTDIR}
$GATK
time gatk SplitNCigarReads --TMP_DIR ${TMP} -R $ASSEMBLY -I HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted.bam -O HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted_split.bam ;
...
id=${file##*/}
id=${file%.*}
time java -jar -Djava.io.TMPdir=$TMP ${PICARD_TOOLS}/AddOrReplaceReadGroups.jar I=HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted_split.bam O=HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted_split_RG.bam RGID=${id} RGLB=${id} RGPL=illumina RGPU=${id} RGSM=${id}
...
samtools flagstat HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted_split_RG.bam.bam > HI.4506.005.NS_Adaptor_9.pool1TR_filtered_sorted_MD_sorted_split_RG.bam.flagstat ;

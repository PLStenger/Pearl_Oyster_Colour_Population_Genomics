...
df <- read.table("input_glm_tuckey_without_iteration_order_for_second_round.txt", header=F, skip=1)
...
registerDoParallel()
  ...
  foreach(i=1:length(unique(df$SNP)), .combine=c) %dopar% {
    sink("input_glm_tuckey_without_iteration_order_for_second_round_results_01.txt", append=TRUE)
